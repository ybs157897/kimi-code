 ...
 *
 * Installs a WorkBuddy-compatible expert plugin, activates it for a real v2
 * Session, verifies the lead profile binding, and deactivates back to the
 * previous profile.
 */
...
describe('server-v2 /api/v2 expert teams', () => {
  ...
  beforeEach(async () => {
    ...
    home = await mkdtemp(join(tmpdir(), 'kimi-server-v2-expert-teams-'));
  ...
  it('discovers, activates, snapshots, and deactivates an expert team', async () => {
    ...
    }>(`/api/v2/sessions/${sessionId}/expert-teams`);
    ...
    const initial = await getJson<{ expert_team: null }>(
      `/api/v2/sessions/${sessionId}/expert-team`,
    );
    ...
    }>(`/api/v2/sessions/${sessionId}/expert-team/activate`, {
      plugin_id: 'delivery-experts',
    });
    ...
    const deactivated = await postJson<{ deactivated: true }>(
      `/api/v2/sessions/${sessionId}/expert-team/deactivate`,
    );
