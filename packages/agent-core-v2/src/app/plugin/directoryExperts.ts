...
/** Directory name scanned for drop-in expert packages under each root. */
...
/**
 * Discovers drop-in expert packages under the given roots. Roots are scanned
 * in order and earlier roots win id collisions.
 */
...
/** Directory experts merged over installed plugin experts; directories shadow same-id plugins. */
export function mergeDirectoryExperts(
  directory: readonly EnabledPluginExpert[],
  plugins: readonly EnabledPluginExpert[],
): readonly EnabledPluginExpert[] {
  const fromDirectories = new Set(directory.map((expert) => expert.pluginId));
  return [...directory, ...plugins.filter((expert) => !fromDirectories.has(expert.pluginId))];
}
