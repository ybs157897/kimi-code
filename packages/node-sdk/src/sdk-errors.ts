/**
 * SDK-local KimiError compatibility class, ErrorCodes registry, and payload
 * serializers.
 *
 * The legacy `@moonshot-ai/agent-core` package defined these symbols.  This
 * module re-implements them so the SDK can stop depending on that package
 * without breaking the public error API (instanceof checks, ErrorCodes string
 * constants, and the KimiErrorPayload wire format).
 *
...
export const KIMI_ERROR_INFO: Record<KimiErrorCode, KimiErrorInfo> = {
  'config.invalid': {
...
// ---------------------------------------------------------------------------
// KimiErrorPayload — wire-safe error shape
// ---------------------------------------------------------------------------
...
/** Build a KimiErrorPayload from a code + message. */
...
/**
 * Rehydrate a KimiErrorPayload into a KimiError so in-process consumers can
 * still use `instanceof`.
 */
...
// ---------------------------------------------------------------------------
// Klient error → KimiError boundary mapper
// ---------------------------------------------------------------------------

import { RPCError, KlientValidationError } from '@moonshot-ai/klient';

/**
 * Map a Klient transport-layer error (RPCError, KlientValidationError) to the
 * SDK's public KimiError protocol.
 *
 * Returns `undefined` when the error is not a recognisable Klient error so the
 * caller can rethrow it raw.
 */
export function mapKlientError(error: unknown): KimiError | undefined {
  if (error instanceof RPCError) {
    // RPCError.code is numeric; map known ranges to KimiErrorCode strings.
    if (error.code >= 100 && error.code < 200) {
      return new KimiError(ErrorCodes.REQUEST_INVALID, error.message, { details: error.details as Record<string, unknown> | undefined });
    }
    if (error.code === 404) {
      return new KimiError(ErrorCodes.SESSION_NOT_FOUND, error.message, { details: error.details as Record<string, unknown> | undefined });
    }
    if (error.code === 409) {
      return new KimiError(ErrorCodes.SESSION_ALREADY_EXISTS, error.message, { details: error.details as Record<string, unknown> | undefined });
    }
    if (error.code >= 500) {
      return new KimiError(ErrorCodes.INTERNAL, error.message, { details: error.details as Record<string, unknown> | undefined });
    }
    return new KimiError(ErrorCodes.REQUEST_INVALID, error.message, { details: error.details as Record<string, unknown> | undefined });
  }
