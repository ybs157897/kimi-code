...
import { reduceExpertTeamFact } from './expertTeamFacts';
...
export function foldWireRecordFacts(
  ...
  let expertTeam: ExpertTeamModeMeta | undefined;
  let expertTeamTouched = false;
  ...
  for (const record of records) {
    switch (record.type) {
      ...
      case 'expert_team.delete': {
        const projection = reduceExpertTeamFact(
          {
            mode: expertTeam,
            touched: expertTeamTouched,
          },
          record,
        );
        if (projection === undefined) break;
        expertTeam = projection.state.mode;
        expertTeamTouched = projection.state.touched;
        if (projection.marker !== undefined) {
          pushMarker(projection.marker, record);
        }
        break;
      }
      case 'task.started':
  ...
  const modesTouched =
    planActive !== undefined || swarmActive !== undefined || expertTeamTouched;
  const meta: TranscriptMeta = {
    ...
    modes: modesTouched
      ? {
          ...
          expertTeam: expertTeamTouched ? expertTeam : base.meta.modes?.expertTeam,
        }
      : base.meta.modes,
  };
