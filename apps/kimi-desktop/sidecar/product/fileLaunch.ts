/**
 * Native "open in app" launch commands for the desktop product facade.
 *
 * Ported from `packages/kap-server/src/lib/fileLaunch.ts` so the sidecar can
 * serve `openFile` / `revealFile` / `openInApp` without depending on the
 * Fastify route layer (docs/plan/desktop-v2-full-integration.md, "原生打开操作":
 * the sidecar resolves the workspace-bounded absolute path via
 * `ISessionFsService.resolvePath()`, then performs the platform operation
 * here). The desktop sidecar is a headless Node sidecar process, so spawning
 * with `detached: true, stdio: 'ignore'` never pops a terminal window on
 * macOS / Windows / Linux — the GUI handlers (`open`, `explorer.exe`,
 * `xdg-open`) open in their own session.
 *
 * The command set is a fixed whitelist (`OPEN_IN_APP_IDS`); arbitrary binaries
 * are never executed. Editor overrides come from the standard `KIMI_CODE_EDITOR`
 * / `VISUAL` / `EDITOR` env vars, exactly like kap-server.
 */
...
function commandExists(command: string, platform: NodeJS.Platform): boolean {
  try {
    if (platform === 'win32') {
      const result = spawnSync('cmd', ['/c', 'where', command], { stdio: 'ignore' });
